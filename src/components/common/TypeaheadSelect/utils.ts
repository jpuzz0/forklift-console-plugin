...
const createNewOption = (
  ...
  const message = typeof createMessage === 'function' ? createMessage(inputValue) : createMessage;
...
const createNoResultsOption = (
  ...
  const message =
    typeof noResultsMessage === 'function' ? noResultsMessage(inputValue) : noResultsMessage;
...
export const generateFilteredOptions = (config: FilterOptionsConfig): TypeaheadSelectOption[] => {
  ...
  if (filtered.length === 0) {
    return [createNoResultsOption(inputValue, noResultsMessage)];
  }
